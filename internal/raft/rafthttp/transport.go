...
func (t *Transport) Send(msgs []raftpb.Message) {
	for _, m := range msgs {
		...
		log.Printf("DEBUG - ignored message %s (sent to unknown peer %s)", m.Type, to)
	}
}
...
func (t *Transport) AddPeer(id types.ID, us []string) {
	...
	log.Printf("INFO - added peer %s", id)
}
...
func (t *Transport) removePeer(id types.ID) {
	...
	log.Printf("INFO - removed peer %s", id)
}
...
func (t *Transport) UpdatePeer(id types.ID, us []string) {
	...
	t.streamProber.Remove(id.String())
	addPeerToProber(t.streamProber, id.String(), us, RoundTripperNameRaftMessage)
	log.Printf("INFO - updated peer %s", id)
}
