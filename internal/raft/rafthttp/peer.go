...
func startPeer(transport *Transport, urls types.URLs, peerID types.ID, fs *stats.FollowerStats) *peer {
	log.Printf("INFO - starting peer %s...", peerID)
	defer log.Printf("INFO - started peer %s", peerID)
		...
		for {
			...
			case mm := <-p.recvc:
				if err := r.Process(ctx, mm); err != nil {
					log.Printf("WARNING - failed to process raft message (%v)", err)
				}
			case <-p.stopc:
				return
			}
...
func (p *peer) send(m raftpb.Message) {
	...
	default:
		...
		if p.status.isActive() {
			log.Printf("WARNING - dropped internal raft message to %s since %s's sending buffer is full (bad/overloaded network)", p.id, name)
		}
		log.Printf("DEBUG - dropped %s to %s since %s's sending buffer is full", m.Type, p.id, name)
	}
}
...
func (p *peer) stop() {
	log.Printf("INFO - stopping peer %s...", p.id)
	defer log.Printf("INFO - stopped peer %s", p.id)
